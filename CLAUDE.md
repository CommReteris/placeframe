...
- **Unity Library cache**: cached via ORAS, keyed by project name + platform + manifest.json hash. The manifest hash ensures package version changes invalidate the cache — stale Library import data can cause `DllNotFoundException` for native plugins even when `.meta` files are correct.
