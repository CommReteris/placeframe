...
10. **GitHub token**: If `GITHUB_TOKEN` is set in the host credentials file (`~/.config/placeframe/credentials`), it is injected into the container via the Incus default profile. Use `gh` CLI for GitHub lookups when available; fall back to `WebFetch` or `WebSearch` if not.
