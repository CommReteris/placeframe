...
REPO_ROOT = Path.cwd()
STATE_FILE = REPO_ROOT / "build" / "package-versions.json"
...
def main(dry_run: bool = typer.Option(False, help="Plan publishes without executing them")) -> None:
    ...
    with ci_step("Compute publish plan"):
        ...
        for name, config in PACKAGES.items():
            old_hash = state[name]["hash"]
            old_version: str = state[name]["version"]
    ...
    with ci_step("Save publish state"):
        new_state = {name: {"version": versions[name], "hash": hashes[name]} for name in PACKAGES}
