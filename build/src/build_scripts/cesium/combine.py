...
def main() -> None:
    ...
    with ci_step("Configure native plugins"):
        (package_path / ".npmignore").write_text(
            "native~/\nBuild~/\nReinterop~/\nTests~/\nDocumentation~/\nSamples~/\n"
            "build-*/\ncodegen-csharp-*/\n.git/\n.github/\n.vs/\n.idea/\n*.user\n"
        )
    ...
    with ci_step("Publish package"):
        ...
        if bash_check(f"npm view {name}@{version} version", cwd=package_path):
            print(f"FATAL: {name}@{version} already published — bump the package-version input")
            sys.exit(1)
        bash("npm publish --access public --provenance --tag latest", cwd=package_path)
