...
Our `write_coi_config()` in `setup_sandbox.py` writes `image = "coi-placeframe"` to the config, but it has no effect. We work around this in `coi_shell.py` by passing `--image coi-placeframe` explicitly.

## Key files

- `scripts/src/scripts/coi_shell.py` — `--image` workaround and comment to update
- `scripts/src/scripts/setup_sandbox.py` — `write_coi_config()` writes the config that doesn't work

## Done when

- [ ] Upstream issue filed on mensfeld/code-on-incus
- [ ] Upstream issue URL linked back into `coi_shell.py` comment and this ticket
- [ ] Once upstream ships a fix, remove the `--image` workaround from `coi_shell.py`
