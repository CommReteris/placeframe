...
CI solves this with an explicit pre-Unity step: `dotnet tool restore && dotnet nugetforunity restore <project>` (see `build-unity.yml` lines 136-139). The COI sandbox image (`agent/coi-placeframe-build.sh`) installs Unity but not the dotnet SDK, so this step can't be replicated locally.

Discovered during T88 when `packages-lock.json` regeneration required manual dotnet installation.

## Key files

- `agent/coi-placeframe-build.sh` — COI image build script, needs dotnet SDK install step
...
- `agent/coi-placeframe-build.sh` installs dotnet SDK 8.0
