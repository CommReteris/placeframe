...
Publish Placeframe packages to npmjs.org under the `org.outernet` scope. Authentication via OIDC trusted publishing (no long-lived tokens). See `agent/plans/t71-plan.md` for original plan (note: package names changed from `com.placeframe.vps*` to `org.outernet.placeframe*` during implementation).
