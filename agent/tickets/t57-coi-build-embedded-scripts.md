...
Track the upstream fix for [mensfeld/code-on-incus#50](https://github.com/mensfeld/code-on-incus/issues/50) — `coi build` currently requires the repo to be cloned locally because build scripts aren't embedded in the binary. Once a release ships with embedded scripts, simplify `setup_sandbox.py`.
...
- `scripts/src/scripts/setup_sandbox.py` — `clone_or_update_coi_repo()`, `COI_REPO_DIR`, `cwd=` on build calls

## Done when

- [ ] New COI release ships with embedded build scripts
- [ ] Remove `clone_or_update_coi_repo()` from `setup_sandbox.py`
